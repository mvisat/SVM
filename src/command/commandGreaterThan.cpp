#include "commandGreaterThan.hpp"

void commandGreaterThan::execute(const std::vector<std::string>& cmd) {
...
void commandGreaterThan::execute(const std::vector<bytecode_t>& cmd) {
...
void commandGreaterThan::execute(index_t rDest, index_t rSrc1, index_t rSrc2) {
    svmMemory->set_register(rDest, svmMemory->get_register(rSrc1) > svmMemory->get_register(rSrc2));
}

void commandGreaterThan::write_bytecode(const std::vector<std::string>& cmd) {
    index_t rDest = parse_register(cmd[1]);
    index_t rSrc1 = parse_register(cmd[2]);
    index_t rSrc2 = parse_register(cmd[3]);

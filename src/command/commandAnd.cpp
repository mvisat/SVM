...
void commandAnd::execute(const std::vector<std::string>& cmd) {
...
void commandAnd::execute(const std::vector<bytecode_t>& cmd) {
...
void commandAnd::write_bytecode(const std::vector<std::string>& cmd) {
    index_t rDest = parse_register(cmd[1]);
    index_t rSrc1 = parse_register(cmd[2]);
    index_t rSrc2 = parse_register(cmd[3]);

...
void commandLessThan::execute(const vector<string>& cmd) {
...
void commandLessThan::execute(const vector<bytecode_t>& cmd) {
...
void commandLessThan::write_bytecode(const vector<string>& cmd) {
    index_t rDest = parse_register(cmd[1]);
    index_t rSrc1 = parse_register(cmd[2]);
    index_t rSrc2 = parse_register(cmd[3]);

#include "commandLoadByte.hpp"

void commandLoadByte::execute(const std::vector<std::string>& cmd) {
...
void commandLoadByte::execute(const std::vector<bytecode_t>& cmd) {
...
void commandLoadByte::execute(index_t rDest, index_t rSrc, address_t C) {
    svmMemory->set_register(rDest,
        svmMemory->get_memory((address_t)svmMemory->get_register(rSrc) + C));
}

void commandLoadByte::write_bytecode(const std::vector<std::string>& cmd) {
    index_t rDest = parse_register(cmd[1]);
    index_t rSrc = parse_register(cmd[2]);
    address_t C = parse_constant(cmd[3]);

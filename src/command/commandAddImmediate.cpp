...
void commandAddImmediate::execute(const std::vector<std::string>& cmd) {
...
void commandAddImmediate::execute(const std::vector<bytecode_t>& cmd) {
...
void commandAddImmediate::write_bytecode(const std::vector<std::string>& cmd) {
    index_t rDest = parse_register(cmd[1]);
    index_t rSrc = parse_register(cmd[2]);
    register_t C = parse_constant(cmd[3]);

...
void commandJumpIfZero::execute(const std::vector<std::string>& cmd) {
...
void commandJumpIfZero::execute(const std::vector<bytecode_t>& cmd) {
...
void commandJumpIfZero::write_bytecode(const std::vector<std::string>& cmd) {
    index_t rDest = parse_register(cmd[1]);
    std::string label = cmd[2];
